          ...
          <div class="c-b-box">
            <div class="body">
              ...
              <div class="form-group">
                ...
                <label for="checkbox">Also delete for media</label>
              </div>
              <div class="d-c-btns">
...
import groupchatmessage from "../Athelete/GroupChatMessageComponent";
import Vue from "vue";
  const firebase = require('../../firebaseConfig.js');

export default {
   components: {chatmessage, chatpage, groupchatmessage},
   props: ['login_user', 'request_coach_chat'],
    data: function() {
    return {
      chat_user_search : '',
      chatusers : [],
      ...
      selected_chat_dt : '',
      active_el: '',
      request_coach_chat_detail : this.request_coach_chat,
  ...
  methods:{
    ...
    fetchUsers:function(){
      ...
      let vm = this;
      let router_url =Vue.http.options.BASE_URL+'/athlete/chatuser';
      axios.get(router_url).then(res=>{
        $('.processing-loader').hide();
        if(res.status==200){
          if(res.data.status){
            this.chatusers = res.data.data.result;
            if(Object.keys(this.request_coach_chat_detail).length > 0){
              this.openuserchat(this.request_coach_chat_detail.chat_uuid);
            }
            this.request_coach_chat_detail = {};
          }else{
            this.chatusers = [];
          }
        }
    ...
    MakeReadMessage:function(user_chat_d, chat_message_type, message_id= ''){
      let vm = this;
      let router_url =Vue.http.options.BASE_URL+'/athlete/chat/readmessage';
    ...
    MakeGroupReadMessage:function(user_chat_d, chat_message_type, message_id= ''){
      let vm = this;
      let router_url =Vue.http.options.BASE_URL+'/athlete/chat/groupreadmessage';
      let formData = new FormData();
      formData.append('chatting_id', user_chat_d);
      formData.append('type', chat_message_type);
      if(chat_message_type == 'R')
    ...
    confirmDelete: function() {
      ...
      $('.processing-loader').show();
      let update_request_url = Vue.http.options.BASE_URL + '/athlete/chatdelete';
      let formData = new FormData();
    ...
    openuserchat:function(el){
      ...
      let selected_chat_user_id = selected_chat[0].one_user_id != this.login_user_detail.id ? selected_chat[0].one_user_id : selected_chat[0].two_user_id;
        ...
        let router_url =Vue.http.options.BASE_URL+'/athlete/chatuserdetail';
