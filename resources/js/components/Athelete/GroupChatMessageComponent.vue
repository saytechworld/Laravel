        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            <div class="msg-group-modify-sec">
                 ...
                 <div class="exit-group-btn"> <button v-if="this.admin == 1" class="btn btn-success" @click="addParticipant">Add Participant</button>   <button v-if="this.admin != 1 && selected_chat_user.status == 1" class="btn btn-danger" @click="exitGroup">Exit Group</button></div>
            </div>
                  ...
                  <div class="card c-b-box">
                    <div class="body text-center">
                      ...
                      <ul class="cmn-ul-list">
                        <li><a :href="link_url+'/athlete/coach/profileDetail/'+user.users.username" class="btn btn-outline-primary"><i class="fa fa-user"></i> <span>Profile</span></a></li>
    ...
    methods: {
      ...
      newline() {
        ...
        viewInfo:function() {
          let vm = this;
          $('.processing-loader').show();
        let router_url = Vue.http.options.BASE_URL + '/athlete/fetchgroupinfo';
        ...
        let vm = this;
          $('.processing-loader').show();
        let router_url = Vue.http.options.BASE_URL + '/athlete/fetchgroupmessage?page='+this.scrollPage;
        axios.post(router_url, {chatting_id: this.selected_chat_uuid}).then(res => {
          if (res.status == 200) {
              $('.processing-loader').hide();
            if (res.data.status) {
      ...
      SubmitMessageFileText: function (message_input, message_type, url) {
        ...
        let send_message_url = Vue.http.options.BASE_URL + '/athlete/sendgroupmessage';
     ...
     openMediaFolderPopup : function (message_id) {
        ...
        addParticipant:function() {
            ...
            $('.processing-loader').show();
            let router_url = Vue.http.options.BASE_URL + '/athlete/fetchnewuser';
            axios.post(router_url, {chatting_id: this.selected_chat_uuid}).then(res => {
                if (res.status == 200) {
                    $('.processing-loader').hide();
                    if (res.data.status) {
        ...
        addMember:function()
        {
            let vm = this;
            $('.processing-loader').show();
            let router_url = Vue.http.options.BASE_URL + '/athlete/addparticipant';
        ...
        removeFromGroup:function(user_id)
        {
            let vm = this;
            $('.processing-loader').show();
            let router_url = Vue.http.options.BASE_URL + '/athlete/removefromgroup';
        ...
        exitGroup:function(user_id)
        {
            let vm = this;
            $('.processing-loader').show();
            let router_url = Vue.http.options.BASE_URL + '/athlete/exitgroup';
            axios.post(router_url, {chatting_id: this.selected_chat_uuid, user_id: user_id}).then(res => {
                if (res.status == 200) {
                    $('.processing-loader').hide();
                    if (res.data.status) {
        ...
        editGroupName:function(user_id)
        {
            ...
            $('.processing-loader').show();
            let router_url = Vue.http.options.BASE_URL + '/athlete/changegroupname';
