<template>
  <div class="row">
    <div class="col-lg-12">
      <div class="chat">
        <div class="chat-header clearfix">
          <div class="row">
            <div class="col-4">
                <a href="javascript:void(0);" data-toggle="modal" data-target="#view_info" v-if="selected_chat_user.deleted_status != 1"> <img :src="link_url+'/images/noimage.jpg'" alt="avatar" />
              ...
              <div class="col-8">
                  <div class="text-right" v-if="selected_chat_user.deleted_status != 1">
                    <ul class="cmn-ul-list">
                      <template v-if="user_session_request.coach_id == login_user_detail.id">
                          <li v-if="user_session_request.status == 2 && user_session_request.request_by == 1"><p class="title">You accepted session request. </p></li>
                          <li v-if="user_session_request.status == 2 && user_session_request.request_by == 2"><p class="title">You sent new session request. </p></li>
                          <template v-if="user_session_request.status == 1">
                              <li><p class="title"> session request.</p></li>
                              <li><button class="btn header-all-session-button accept"  data-toggle="modal" data-target="#acceptRequest" @click="session_request = user_session_request"><i class="fa fa-check"></i></button>
                                  <button class="btn header-all-session-button deny" v-on:click="RejectAtheleteRequest(session_request = user_session_request)"><i class="fa fa-close"></i></button></li>
                          </template>
                          <template  v-if="user_session_request.status == 4">
                              <li><p class="title"> Please start your session</p></li>
                              <li><button class="btn session-start-btn semilar-btn" v-on:click="StartAtheleteSession(session_request = user_session_request)"><img :src="link_url+'/images/session.svg'" /><br><span class="session-content">Start Session</span></button></li>
                          </template>
                          <template  v-if="user_session_request.status == 6">
                              <li><p class="title">Session started</p></li>
                              <li><button class="btn header-all-session-button end-btn semilar-btn" v-on:click="CompleteAtheleteSession(session_request = user_session_request)"><img :src="link_url+'/images/end-session.svg'" /><br><span class="session-content">End Session</span></button></li>
                          </template>
                      </template>
                      <template v-if="user_session_request.athelete_id == login_user_detail.id">
                          ...
                          <template v-if="user_session_request.status == 2">
                              ...
                              <div class="session_btn">
                                  <button class="btn header-all-session-button accept"  data-toggle="modal" data-target="#acceptCoachSessionPriceRequest" @click="session_request = user_session_request"><i class="fa fa-check"></i></button>
                      ...
                      <template v-if="process_requests_count == 0 && selected_chat_user.role_type == 'coach'">
                          <li><button class="btn header-all-session-button book semilar-btn" @click="SendCoachSessionRequest"><img :src="link_url+'/images/book.svg'" /><br><span class="session-content">Book Session</span></button></li>
                      </template>
                      <template v-if="process_requests_count == 0">
                          <li><button class="btn session-start-btn semilar-btn"  data-toggle="modal" data-target="#sendSessionRequest" @click="session_request = 2"><img :src="link_url+'/images/session.svg'" /><br><span class="session-content">Start Session</span></button></li>
                      </template>
                      <template>
                          <li><a :href="link_url+'/coach/join_meeting/'+this.selected_chat_uuid" target="_blank" class="btn session-start-btn semilar-btn" ><img :src="link_url+'/images/zoom.svg'" /><br><span class="session-content">ZOOM JOIN</span></a></li>
                      </template>
                  </ul>
                  </div>
            </div>
                ...
                <div class="message my-message">
                  ...
                  <div v-if="user_message.message_type == 12" class="all-chat-msg-pos document_box black-chat">
                    ...
                    <div v-if="user_message.message_type == 4" class="all-chat-msg-pos">
                      <p>You received a session request</p>
                       <span class="timing">{{ user_message.message_created_time }}</span>
                    </div>
                    ...
                    <div v-if="user_message.message_type == 7" class="all-chat-msg-pos">
                      <p>Your session price request has been accepted.</p>
                       <span class="timing">{{ user_message.message_created_time }}</span>
                    </div>
                    <div v-if="user_message.message_type == 8" class="all-chat-msg-pos">
                      <p>Your session  price request has been declined.</p>
                       <span class="timing">{{ user_message.message_created_time }}</span>
                    </div>
              ...
              <div v-else>
                <div class="message other-message float-right">
                  ...
                  <div v-if="user_message.message_type == 12" class="all-chat-msg-pos document_box black-chat">
                    ...
                    <div v-if="user_message.message_type == 4" class="all-chat-msg-pos">
                      <p >You sent a session request</p>
                      <span class="timing">{{ user_message.message_created_time }}</span>
                    </div>
                    <div v-if="user_message.message_type == 5" class="all-chat-msg-pos">
                      <p >You accepted a session request</p>
                      <span class="timing">{{ user_message.message_created_time }}</span>
                    </div> 
                    <div v-if="user_message.message_type == 6" class="all-chat-msg-pos">
                      <p >You rejected a session request</p>
                      <span class="timing">{{ user_message.message_created_time }}</span>
                    </div>  
                    <div v-if="user_message.message_type == 7" class="all-chat-msg-pos">
    ...
    <div class="col-lg-4">
      ...
      <div class="modal" id="acceptCoachSessionPriceRequest" tabindex="-1" role="dialog" ref="acceptRequestPriceModal">
          <div class="modal-dialog" role="document">
              <div class="modal-content">
                  <form class="uk-padding">
                      <div class="modal-header">
                          <h4 class="title" id="paymentLabel">Payment</h4>
                      </div>
    ...
    <div class="modal add_event_category_modal" id="acceptRequest" tabindex="-1" role="dialog" ref="acceptAtheleteSessionRequest">
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <form  method="post"  v-on:submit.prevent="AcceptAtheleteRequest">
            <div class="modal-header">
              <h4 class="title" id="defaultModalLabel">Accept Athlete Request</h4>
              <button type="button" class="btn btn-simple modal-close-btn" data-dismiss="modal"><i class="icon-close"></i></button>
            </div>
            <div class="modal-body">
              <div class="form-group">
                <div class="form-line">
                  <input type="text" class="form-control" placeholder="Please put here Session price" v-model="session_price" name="session_price">
                </div>
              </div>
            </div>
            <div class="modal-footer">
    ...
    <div class="modal add_event_category_modal" id="sendSessionRequest" tabindex="-1" role="dialog" ref="sendRequestModal">
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <form  method="post"  v-on:submit.prevent="AcceptAtheleteRequest">
            <div class="modal-header">
              <h4 class="title" id="sessionRequestModal">Price for the session</h4>
              <button type="button" class="btn btn-simple modal-close-btn" data-dismiss="modal"><i class="icon-close"></i></button>
            </div>
            <div class="modal-body">
              <div class="form-group">
          ...
          <div class="modal-body">
            <div class="card c-b-box">
              <div class="body text-center">
                ...
                <p v-if="selected_chat_user.role_type != 'athlete'">Experience: {{ selected_chat_user.user_details ? selected_chat_user.user_details.experience : '0' }} Years</p>
                <ul class="cmn-ul-list">
                  <li><a :href="link_url+'/coach/athlete/profileDetail/'+selected_chat_user.username" class="btn btn-outline-primary"><i class="fa fa-user"></i> <span>Profile</span></a></li>
                </ul>
              </div>
            </div>
          </div>
...
<script>
  ...
  export default {
    ...
    created: function () {
      ...
      this.chat_ref.on('value', (snapshot) => {
        snapshot.forEach((doc) => {
          let item = doc.val();
          item.key = doc.key;
          if (this.selected_chat_dt.id == item.chat_id) {
              ...
              } else if(item.delete_two && item.delete_two == this.login_user_detail.id) {
                  var result = $.grep(messages, function(e){
                      return e.id != item.id;
                  });
                  this.user_messages = result;
              } else if(item.delete_one && item.delete_one == this.login_user_detail.id) {
                  var result = $.grep(messages, function(e){
                      return e.id != item.id;
                  });
                  this.user_messages = result;
              }else if(!item.delete_everyone && !item.delete_one && !item.delete_two) {
      ...
      this.session_requests_ref.on('value', (requests_ref_snapshot) => {
        requests_ref_snapshot.forEach((requests_ref_doc) => {
          ...
          if (this.selected_chat_dt.id == requests_ref_item.chat_id) {
            this.checkPendingApppendSession(requests_ref_item);
          }
    ...
    mounted() {
        this.setUpStripe();
        this.setUpPlupload();
        $(this.$refs.acceptRequestPriceModal).on('hidden.bs.modal', this.resetAcceptPriceFormData);
        $(this.$refs.acceptAtheleteSessionRequest).on('hidden.bs.modal', this.resetAtheleteSessionFormData);
        $(this.$refs.sendRequestModal).on('hidden.bs.modal', this.resetSendRequestFormData);
    ...
    methods: {
      ...
      newline() {
        ...
        paymentAuthorized: function (data) {
            ...
            let send_message_url =Vue.http.options.BASE_URL+'/coach/payment/paypal';
        ...
        paymentCompleted: function(data) {
            let send_message_url =Vue.http.options.BASE_URL+'/coach/paymentcomplete/paypal';
            let formData = new FormData();
            ...
            axios.post(send_message_url,formData,{headers: {'Content-Type': 'multipart/form-data'}}).then( res => {
                if(res.status==200){
                    ...
                    if(res.data.status){
                        $('#acceptCoachSessionPriceRequest').modal().hide();
        ...
        resetAtheleteSessionFormData() {
          this.session_price='';
        },

        resetSendRequestFormData() {
            this.session_price='';
        },
      ...
      fetchUserChats: function () {
        let vm = this;
          $('.processing-loader').show();
        let router_url = Vue.http.options.BASE_URL + '/coach/fetchusermessage?page='+this.scrollPage;
      ...
      fetchUserSessionRequests: function () {
        let vm = this;
        let router_url = Vue.http.options.BASE_URL + '/coach/fetchusersessionrequest';
        axios.post(router_url, {chatting_id: this.selected_chat_uuid}).then(res => {
          if (res.status == 200) {
            if (res.data.status) {
              ...
              this.process_requests_count = res.data.data.process_count;
              this.myItems = [
                  {
                      "name": "Session Request ",
                      "description": this.user_session_request.chat_session_uuid,
                      "quantity": "1",
            ...
            if (type == 'N') {
                this.stripe.createToken(this.cardNumber).then((result) => {
                    if (result.error) {
                        ...
                        let send_message_url =Vue.http.options.BASE_URL+'/coach/payment/post';
                        let formData = new FormData();
        ...
        fetchUserCards: function () {
            let vm = this;
            let router_url = Vue.http.options.BASE_URL + '/coach/fetchusercard';
            axios.get(router_url).then(res => {
                if (res.status == 200) {
                    if (res.data.status) {
      ...
      SubmitMessageFileText: function (message_input, message_type, url=null) {
        if(url) {
            this.tempUrl = url;
        }
        ...
        let send_message_url = Vue.http.options.BASE_URL + '/coach/sendusermessage';
        let formData = new FormData();
      ...
      RejectAtheleteRequest: function () {
        let vm = this;
        $('.processing-loader').show();
        let update_request_url = Vue.http.options.BASE_URL + '/coach/updateusersessionrequest';
        let formData = new FormData();
        formData.append('session_request_id', this.session_request.chat_session_uuid);
        formData.append('chatting_id', this.selected_chat_uuid);
      ...
      AcceptAtheleteRequest(event) {
        if (this.session_price == '') {
          toastr.error('Session Price should not be blank');
          return false;
        }

        if(isNaN(this.session_price)) {
          toastr.error('The session price Should be float type');
          return false;
        }
        ...
        let update_request_url = Vue.http.options.BASE_URL + '/coach/updateusersessionrequest';
        let formData = new FormData();
        formData.append('session_price', this.session_price);
        if (this.session_request == 2) {
          formData.append('request_by', 2);
        } else {
          formData.append('session_request_id', this.session_request.chat_session_uuid);
          formData.append('request_by', 1);
        }
        formData.append('chatting_id', this.selected_chat_uuid);
        formData.append('request_status', 2);
        axios.post(update_request_url, formData, {headers: {'Content-Type': 'multipart/form-data'}}).then(res => {
          if (res.status == 200) {
            $('.processing-loader').hide();
            if (res.data.status) {
      ...
      RequestSessionObjUpdate:function(current_request_data)
      {
        if (this.user_session_request.id == current_request_data.id) {
            ...
            this.myItems = [
                {
                    "name": "Session Request",
                    "description": current_request_data.chat_session_uuid,
                    "quantity": "1",
      ...
      checkPendingApppendSession:function(current_request_data)
          {
            if (current_request_data.status == 1 || current_request_data.status == 2 || current_request_data.status == 4 || current_request_data.status == 6) {
                this.process_requests_count = 1;
            } else {
                this.process_requests_count = 0;
            }
      ...
      StartAtheleteSession : function () {
        let vm = this;
        $('.processing-loader').show();
        let update_request_url = Vue.http.options.BASE_URL + '/coach/usersession_start';
        let formData = new FormData();
        formData.append('session_request_id', this.session_request.chat_session_uuid);
        formData.append('chatting_id', this.selected_chat_uuid);
        formData.append('request_status', 6);
        axios.post(update_request_url, formData, {headers: {'Content-Type': 'multipart/form-data'}}).then(res => {
          if (res.status == 200) {
            $('.processing-loader').hide();
            if (res.data.status) {
      ...
      CompleteAtheleteSession : function () {
        let vm = this;
        $('.processing-loader').show();
        let update_request_url = Vue.http.options.BASE_URL + '/coach/usersession_completed';
        let formData = new FormData();
        formData.append('session_request_id', this.session_request.chat_session_uuid);
        formData.append('chatting_id', this.selected_chat_uuid);
        ...
        axios.post(update_request_url, formData, {headers: {'Content-Type': 'multipart/form-data'}}).then(res => {
          ...
          SendCoachSessionRequest:function(){
            let vm = this;
            if(this.process_requests_count == false){
                $('.processing-loader').show();
                let router_url =Vue.http.options.BASE_URL+'/coach/sendsessionrequest';
      ...
      RejectCoachSessionPriceRequest:function(){
            let vm = this;
              $('.processing-loader').show();
            let router_url =Vue.http.options.BASE_URL+'/coach/rejectusersessionrequest';
            let formData = new FormData();
            formData.append('session_request_id', this.session_request.chat_session_uuid);
            formData.append('chatting_id', this.selected_chat_uuid);
