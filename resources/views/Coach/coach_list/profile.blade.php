...
$user_games = $user->coach_games()->groupBy('title')->pluck('title')->toArray();

$user_languages = $user->user_spoken_languages()->pluck('title')->toArray();

$user_skills = $user->coach_games_skills()->pluck('title')->toArray();

$user_videos = $user->videos()->where('file_type', 1)->where('privacy', 1)->get();
$user_photos = $user->videos()->where('file_type', 2)->where('privacy', 1)->get();
    ...
    <div class="col-lg-4 col-md-12">
        <div class="card profile-header">
            ...
            <div class="body text-center">
                ...
                <div class="m-t-15">
                    <a href="{{ route('coach.chat.startuserchating',$user->user_uuid) }}" class="btn btn-outline-secondary m-b-10">Chat with Coach</a>
                </div>
                @if($privacy == 1)
                ...
                <div class="body">
                    <ul class="list-unstyled categories-clouds m-b-0">
                        @foreach($user_skills as $user_skill)
                            <li><a href="javascript:void(0);">{{ $user_skill }}</a></li>
                        @endforeach
                    </ul>
                </div>
            @endif
                            ...
                            <div class="col-lg-3 col-md-6 col-sm-6">
                                <div class="card c-b-box">
                                    <div class="file">
                                        <div class="hover">
                                            ...
                                            <div class="chat-option">
                                                <div class="dropdown">
                                                    <button class="dropdown-toggle dropdown-open" data-toggle="dropdown"></button>
                                                    <div class="dropdown-menu">
                                                        <a href="{{ route('frontend.downloadfile',['type'=>'U', 'id'=> $user_photo->id] ) }}" class="dropdown-item"> Download </a>
                                                    </div>
                                                </div>
                                            </div>
                                        ...
                                        <div class="image video-bx">
                                            <img class="image-click" src="{!! $user_photo->aws_video_folder_path !!}">
                                        </div>
                                        <div class="file-name">
                                            <p>Title: <strong>{!! $user_photo->title !!}</strong></p>
                                            <p>Privacy: <strong>{{ $user_photo->privacy == 1 ? 'Public' : 'Private' }}</strong></p>
                                            <p>Description : {!! $user_photo->description !!}</p>
                                            <p>Tag: {!! $user_photo->video_tags()->count() > 0 ? implode(', ', $user_photo->video_tags()->pluck('title')->toArray()) : ""  !!}</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
