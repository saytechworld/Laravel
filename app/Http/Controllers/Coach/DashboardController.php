...
use DB,Exception,Hash,Validator,File;
use  App\Http\Requests\Coach\ChangePasswordRequest;
use  App\Http\Requests\Coach\UpdateProfileRequest;
use  App\Http\Requests\Coach\UpdateGameExperienceRequest;
use App\Models\User;
...
use App\Services\Coach\CardInformationTrait;
use Kreait\Firebase;    
...
{
    use CardInformationTrait;

    public function index()
    {
        $earning = SessionRequest::where('coach_id', auth()->id())->where('status', 7)->sum('session_price');
        $total_session = SessionRequest::whereRaw("(coach_id = ".auth()->id()." OR athelete_id = ".auth()->id().")")->count();
        ...
        return view('Coach.dashboard',
            compact('earning','total_session', 'event_colors', 'events', 'videos', 'photos', 'users', 'chats', 'sessions'));
    }

    public function fetchProfile()
    {
    	$loginuser = auth()->user();
    	return view('Coach.profile',compact('loginuser'));
    }
    ...
    public function updateGameSkill(UpdateGameExperienceRequest $request)
    {
        DB::beginTransaction();
        try {
            $input = $request->all();
        ...
        try {
            if($request->ajax()){
                ...
                if($loggedInUser->fill(['deleted_status' => 1])->save()){
                    ...
                    $user_parent_folder = UserFolder::whereNull('user_folder_id')->where('user_id',auth()->id())->first();
                    $parent_folder_slug =  "";
