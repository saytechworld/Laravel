...
trait UserChatTrait
{
    ...
    public function startChatting(Request $request)
    {
        ...
        try{
            ...
            $receiver = User::whereHas('roles',function($query){
                ...
                ->where('test_user',$test_user)->whereRaw("(user_uuid = '".$input['user_uuid']."' AND status = 1 AND confirmed = 1 )")->first();
            ...
            if (!empty($receiver)) {
                ...
                if (count($roles) > 0 && (in_array(3, $roles) || in_array(4, $roles))) {
                    ...
                    if(empty($checkchat)){
                        if ($receiver->deleted_status == 1) {
                            $this->WebApiArray['status'] = true;
                            $this->WebApiArray['data'] = null;
                            $this->WebApiArray['deleted_status'] = 1;
                            $this->WebApiArray['message'] = "This user doesn't belong to our database";
                    ...
                    $chat = Chat::whereRaw("( (one_user_id = ".$user->id." AND  two_user_id = ".$receiver->id.") OR (two_user_id = ".$user->id." AND  one_user_id = ".$receiver->id.")    )")
                        ->with(['one_users:id,name,username,user_uuid,deleted_status','two_users:id,name,username,user_uuid,deleted_status', 'group_users'])->withCount(['chat_messages' => function($query) use($user){
                    ...
                    if (!empty($chat)) {
                        ...
                        $this->WebApiArray['deleted_status'] = 0;
    ...
    public function fetchChatUser(Request $request)
    {
        try {
            ...
            $chatusers->map(function ($chat) {
                ...
                $chat->updated_at = $chat->updated_time;
                $chat->makeHidden(['updated_time']);
            });
        ...
        try {
            ...
            if($user_messages->count() > 0){
                $user_messages->map(function ($user_message) {
                    $user_message->makeHidden(['user_id', 'read_flag','delete_one', 'delete_two', 'delete_everyone','message_created_date']);
    ...
    public function fetchGroupMessage(Request $request)
    {
        try {
            ...
            if (!empty($group_user)) {
                ...
                if($user_messages->count() > 0){
                    $user_messages->map(function ($user_message) {
                        $user_message->makeHidden(['user_id', 'read_flag','delete_one', 'delete_two', 'delete_everyone','message_created_date','group_delete_message','group_read_message','updated_at']);
                        $user_message->senders->makeHidden(['role_type', 'remaining_balance','total_balance', 'user_details', 'roles']);
                    });
        ...
        try {
            ...
            if($checkchat->fill($chat_data)->save()){
                ...
                if($createdmessage = $checkchat->chat_messages()->create($chat_message_data)){
                    ...
                    sendFcmNotification($reciever_id,'Asportcoach', 'You have a new message', "messenger", $user->id);
    ...
    public function fetchGroupInfo(Request $request) {
        try {
            ...
            $users->active_group_users->map(function ($user) {
                $user->makeHidden(['id','user_id','chat_id','created_at','updated_at']);
    ...
    public function mediaArchive(Request $request) {
        try {
            ...
            $user = $this->getAuthenticatedUser();
            $videos = Video::selectRaw('id, user_id, file_name, title,thumbnail,file_type')->where('user_id', $user->id)->where('file_type', 1)->whereNull('user_folder_id')->orderBy('updated_at','DESC')->get();
            $photos = Video::selectRaw('id, user_id, file_name, title,thumbnail,file_type')->where('user_id', $user->id)->where('file_type', 2)->whereNull('user_folder_id')->orderBy('updated_at','DESC')->get();
    ...
    public function fetchFolderMediaArchive(Request $request) {
        try {
            ...
            $media = Video::selectRaw('id, user_id, file_name,title ,thumbnail,file_type,user_folder_id')->where('user_id', $user->id)->where(['user_folder_id' => $input['folder_id']])->orderBy('updated_at','DESC')->get();
        ...
        try {
                ...
                if (!empty($check_chat)) {
                    ...
                    if ( $check_chat->fill($delete_chat)->save()) {
                        ...
                        $this->WebApiArray['status'] = true;
                        $this->WebApiArray['message'] = 'Message Deleted Successfully.';
                        $this->WebApiArray['data'] = $check_chat;
                        return response()->json($this->WebApiArray);
                    }
    ...
    public function checkMeeting(Request $request) {
        ...
        try {
            ...
            $chat = Chat::where('chat_uuid', $input['chatting_id'])->first();

            $meeting = ChatMeeting::where('chat_id', $chat->id)->first();
            if (!empty($meeting)){
                $this->WebApiArray['status'] = true;
                $this->WebApiArray['message'] = 'Meeting Exist';
                $this->WebApiArray['data'] = $meeting;
                $this->WebApiArray['statusCode'] = 0;
                return response()->json($this->WebApiArray);
            } else {
                $this->WebApiArray['status'] = true;
                $this->WebApiArray['message'] = 'Meeting Not found';
                $this->WebApiArray['data'] = null;
                $this->WebApiArray['statusCode'] = 1;
                return response()->json($this->WebApiArray);
            }
    ...
    public function createMeeting(Request $request) {
        ...
        try {
            ...
            $meeting_input['meeting_id'] = $input['meeting_id'];
            $meeting_input['meeting_password'] = $input['password'];
            if (!empty($meeting)){
                if ($meeting->fill($meeting_input)->save()){
                    DB::commit();
                    $this->WebApiArray['status'] = true;
                    $this->WebApiArray['message'] = 'Meeting updated successfully';
                    $this->WebApiArray['data'] = $meeting;
                    $this->WebApiArray['statusCode'] = 0;
                    return response()->json($this->WebApiArray);
                }
            ...
            $meeting_input['chat_id'] = $chat->id;

            $meeting = new ChatMeeting();
            if ($meeting->fill($meeting_input)->save()){
                DB::commit();
                $this->WebApiArray['status'] = true;
                $this->WebApiArray['message'] = 'Meeting created successfully';
                $this->WebApiArray['data'] = $meeting;
                $this->WebApiArray['statusCode'] = 0;
                return response()->json($this->WebApiArray);
            }
        ...
        try {
            ...
            if (empty($meeting->attendants)){
                ...
                if($createdmessage = $chat->chat_messages()->create(['user_id' => auth()->id(), 'message_type' => 11, 'message_uuid' => Str::uuid()->toString() ])) {
                    ...
                    $this->sendFirebaseNotification($sentmessage, 'chats');
                }
            }
            ...
            $this->WebApiArray['status'] = true;
            $this->WebApiArray['message'] = 'Meeting Joined Successfully';
            return response()->json($this->WebApiArray);
        } catch (Exception $e) {
            DB::rollBack();
            $this->WebApiArray['message'] = $e->getMessage();
    ...
    public function leaveMeeting(Request $request) {
        ...
        try {
            ...
            $this->WebApiArray['status'] = true;
            $this->WebApiArray['message'] = 'Meeting Leave Successfully';
            return response()->json($this->WebApiArray);
        } catch (Exception $e) {
            DB::rollBack();
            $this->WebApiArray['message'] = $e->getMessage();
