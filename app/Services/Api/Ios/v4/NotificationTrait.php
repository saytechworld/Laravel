...
trait NotificationTrait
{
    ...
    public function notification(Request $request) {
        try {
            ...
            $notifications->map(function ($notification) {
                $notification->created_at = $notification->created_at->addHour(2)->format('Y-m-d H:s:i');
    ...
    public function unreadNotificationList(Request $request) {
        try {
            ...
            $notification->map(function ($notificationDt) {
                $notificationDt->created_at = $notificationDt->created_at->addHour(2)->format('Y-m-d H:s:i');
            });
    ...
    public function getUnreadNotificationCount(Request $request) {
        try {
            ...
            $notification = Notification::where('to_user_id', $user->id)->where('read_all', 0)->count();
                    ...
                    if(!empty($readnotification)){
                        ...
                        if ($notification->type == 3) {
                            ...
                            if ($input['action'] == 1) {
                                ...
                                $team_notification_message = auth()->user()->name.' has accepted your invitation to '.$team->title.' team ';
                            } else {
                                $team_notification_message = auth()->user()->name.' has rejected your invitation to '.$team->title.' team';
                            }
                            $team_notification = $team->notifications()->create(['type' => 4, 'from_user_id' => auth()->id(), 'to_user_id' => $team->user_id, 'data' => $team_notification_message, 'notification_uuid' => Str::uuid()->toString()]);
                            self::sendFirebaseNotification($team_notification, 'user_notifications');
                            sendFcmNotification($team->user_id, 'Asportcoach', $team_notification_message, 'team');
                        } else {
    ...
    public function notificationSetting(Request $request)
    {
        ...
        try {
            ...
            if ($user->fill(['notification_setting' => $input['action']])->save()) {
                DB::commit();
                $this->WebApiArray['data'] = $input['action'];
                $this->WebApiArray['status'] = true;
