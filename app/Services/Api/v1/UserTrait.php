...
trait UserTrait
{
    public function Login(Request $request)
    {
        try {
                ...
                $user->makeHidden(['username', 'user_uuid','product_tour','privacy','created_at','updated_at','deleted_status','notification_setting','total_balance','remaining_balance','roles','user_details']);
    ...
    public function homepageCoach()
    {
        try {
            ...
            $events = Event::selectRaw('id, title, description, color_code, event_datetime,end_datetime')->whereHas('event_attendants',function($query) use ($user){
            ...
            $sessions = $sessions->selectRaw('id, athelete_id, coach_id')->whereRaw("(coach_id = ".$user->id." OR athelete_id = ".$user->id.")")
                ->with('athelete_user:id,name')
                ->with('coach_user:id,name')
                ->whereIn('status', [4,6])
