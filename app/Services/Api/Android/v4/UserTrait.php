...
trait UserTrait
{
    ...
    public function getProfile()
    {
        try {
            ...
            $user->user_details->makeHidden(['id','user_id','image','zipcode_id','created_at','updated_at','user_profile_image']);
            ...
            $user->games = $user->coach_games->unique();

            $user->makeHidden(['coach_games']);
    ...
    public function updateAccountSetting(Request $request)
    {
        ...
        try {
            ...
            if($user->fill(['name' => $input['name'], 'privacy' => $input['privacy']])->save()){
                DB::commit();
                $this->WebApiArray['status'] = true;
                $this->WebApiArray['message'] = 'Information updated successfully.';
                return response()->json($this->WebApiArray);
            }
            throw new Exception("Error Processing Request", 1);
        } catch (Exception $e) {
    ...
    public function updateAboutSetting(Request $request)
    {
        ...
        try {
            ...
            $detail['gender'] = $input['gender'];
            $detail['about'] = $input['about'];
            $detail['experience'] = $input['user_experience'];

            $user_details = UserDetail::firstOrCreate(['user_id'=>$user->id]);
            if ($user_details->fill($detail)->save()){
                DB::commit();
                $this->WebApiArray['status'] = true;
                $this->WebApiArray['message'] = 'Account Setting updated successfully.';
                return response()->json($this->WebApiArray);
            }
            throw new Exception("Error Processing Request", 1);
        } catch (Exception $e) {
    ...
    public function androidUpdateCoachSkill(Request $request)
    {
        ...
        try {
            ...
            DB::commit();
            $this->WebApiArray['status'] = true;
            $this->WebApiArray['message'] = 'Skills updated successfully.';
            return response()->json($this->WebApiArray);
    ...
    public function updateCoachLanguage(Request $request)
    {
        DB::beginTransaction();
        try {
            if(!$request->isMethod('post'))
            ...
            DB::commit();
            $this->WebApiArray['status'] = true;
            $this->WebApiArray['message'] = 'Language updated successfully.';
            return response()->json($this->WebApiArray);
        } catch (Exception $e) {
            DB::rollBack();
            $this->WebApiArray['message'] = $e->getMessage();
    ...
    public function homepage()
    {
        try {
            ...
            }])->with(['one_users:id,name,username,user_uuid,deleted_status','two_users:id,name,username,user_uuid,deleted_status', 'group_users'])
            ...
            $this->WebApiArray['data']['subscription'] = $trial_period;
            return response()->json($this->WebApiArray);
