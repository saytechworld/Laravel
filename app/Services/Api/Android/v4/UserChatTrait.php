            ...
            if (!empty($receiver)) {
                ...
                if (count($roles) > 0 && (in_array(3, $roles) || in_array(4, $roles))) {
                    ...
                    $chat = Chat::whereRaw("( (one_user_id = ".$user->id." AND  two_user_id = ".$receiver->id.") OR (two_user_id = ".$user->id." AND  one_user_id = ".$receiver->id.")    )")
                        ->with(['one_users:id,name,username,deleted_status','two_users:id,name,username,deleted_status','meeting:chat_id,meeting_id,meeting_password', 'group_users'])->withCount(['chat_messages' => function($query) use($user){
